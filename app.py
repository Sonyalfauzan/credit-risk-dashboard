...
def prepare_features_for_model(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    - Setelah itu, rename nama kolom supaya match dengan nama fitur
      yang dipakai saat training model XGBoost (mis. Unnamed_0, verification_status_Source_Verified).
    """
    df = df_raw.copy()

    # Pastikan semua kolom fitur ada di data mentah
    ...
    # Ambil hanya kolom fitur, dengan urutan sesuai FEATURES
    df = df[FEATURES].copy()
    ...
    # XGBoost model dilatih dengan nama fitur yang sedikit beda:
    # - 'Unnamed: 0'            -> 'Unnamed_0'
    # - 'verification_status_Source Verified' -> 'verification_status_Source_Verified'
    # Untuk amannya, kita ganti spasi dan ':' menjadi '_' di semua nama kolom.
    renamed_cols = {
        col: col.replace(" ", "_").replace(":", "_")
        for col in df_scaled.columns
    }
    df_scaled = df_scaled.rename(columns=renamed_cols)
